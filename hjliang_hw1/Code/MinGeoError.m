...
function [ks, K, Rs, ts] = decodeParams(params_encoded, N)
%% K
K = zeros(3, 3);
...
%% R
q = reshape(params_encoded(1:4*N), [4, 1, N]);
Rs = q2R(q);
params_encoded = params_encoded(4*N+1:end);
%% ts
ts = reshape(params_encoded(1:3*N), [3, 1, N]);
params_encoded = params_encoded(3*N+1:end);
%% ks
ks = params_encoded;
